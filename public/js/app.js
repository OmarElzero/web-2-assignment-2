...
function createWatchlistEditPanel(movie) {
  ...
  panel.innerHTML = `
    ...
    <label class="watchlist-edit__label">Rating (1–10)
      <input type="number" class="watchlist-edit__input" data-field="rating" min="1" max="10" value="${movie.rating ?? ""}" placeholder="Optional">
    </label>
...
async function renderProfilePage() {
  ...
  if (reviewsCount) reviewsCount.textContent = String(countUserReviews());
...
function countUserReviews() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key.startsWith("movieAppReviews_")) {
      const reviews = JSON.parse(localStorage.getItem(key));
      count += reviews.filter(r => r.userName === user.name).length;
    }
  }
  return count;
}

function getReviewsKey(movieId) {
  return `movieAppReviews_${movieId}`;
}

function getMovieReviews(movieId) {
  const key = getReviewsKey(movieId);
  const raw = localStorage.getItem(key);
  return raw ? JSON.parse(raw) : [];
}

function saveMovieReview(movieId, review) {
  const user = getLoggedInUser();
  ...
  localStorage.setItem(getReviewsKey(movieId), JSON.stringify(reviews));
  return true;
}
...
function wireReviewForm(movieId) {
  ...
  if (form) {
    form.addEventListener("submit", event => {
      ...
      const success = saveMovieReview(movieId, { rating, text });
      if (success) {
